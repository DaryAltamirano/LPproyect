...
export default {
    data(){
        return {
        denuncianteId:this.$route.params.desaparecidoId,
        form: {
        Nombre:'',
        Appellido:'',
        Telefono:0,
     ...
     methods: {
        subir(){
             ...
             axios.put(path,this.form).then((response)=> {
                ...
                alert("Desaparecido actualizado exitosamente")
            }).catch((error)=>{
                console.error(error)
        ...
        getDenunciante(){
            const path =  `http://127.0.0.1:8000/api/v1.0/PersonaDesaparecido/${this.desaparecidoId}/`
